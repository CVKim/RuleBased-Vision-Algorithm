...
namespace tk {
	...
	spline::spline(const std::vector<double>& x, const std::vector<double>& y) : m_x(x), m_y(y) {
...
cv::Mat imread_unicode(const std::wstring& wpath) {
...
bool imwrite_unicode(const std::wstring& wpath, const cv::Mat& image) {
...
int main() {
	...
	fs::path base_path = L"E:/Dev/test/ky_contour_extractor/test2";
	...
	if (files.empty()) {
		logger.log(L"No image to process.");
		return 0;
	}

	logger.log(L"Total " + std::to_wstring(files.size()) + L"Automatically process N images...");
	...
	for (const auto& src_path : files) {
		...
		wss << std::fixed << std::setprecision(2)
			...
			<< L" | ProcTime=" << std::setprecision(3) << elapsed_seconds.count() << L"s"; // 처리 시간 추가
		logger.log(wss.str());
	}
